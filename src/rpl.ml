...
let get_pars ifile ofile wr hr pszr tr sr ct ctype loops verbose =
...
let main () =
  ...
  get_pars ifile ofile wr hr pszr test s_ratio ccl_test ccl_type loops
    verbose;
  let _ = Rinit.r_init in
  Printf.printf "w=%d, h=%d, psz=%d, test=%b, sratio=%d, ccl type=%d, ccl test=%B, loops=%d, verbose=%d\n"
    !wr !hr !pszr !test !s_ratio !ccl_type !ccl_test !loops !verbose;
  if !test = true then
