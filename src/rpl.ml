...
let get_pars ifile ofile wr hr pszr tr sr =
...
let main () =
  ...
  get_pars ifile ofile wr hr pszr test s_ratio;
  let _ = Rinit.r_init in
  Printf.printf "w=%d, h=%d, psz=%d, test=%b, sratio=%d\n"
    !wr !hr !pszr !test !s_ratio;
  if !test = true then
